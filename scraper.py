    ...
    async with async_playwright() as p:
        ...
        with open('jobs.csv', 'w', newline='', encoding='utf-8') as file:
            ...
            for page_num in range(1, 6):
                ...
                for job in jobs:
                    title = await job.query_selector('a[data-automation="jobTitle"]')
                    href = await title.get_attribute('href') if title else None

                    # build clean URL
                    clean_url = None
                    if href:
                        clean_url = "https://au.seek.com" + href.split('?')[0]
                    ...
                    writer.writerow([title_text, company_text, location_text, salary_text, clean_url])
